...
import { FlashQuiz as FlashQuizType, BOX_COLORS } from '../utils/types';

interface FlashQuizProps {
  quiz: FlashQuizType;
  onAnswer: (quizId: number, isCorrect: boolean) => void;
}

export default function FlashQuiz({ quiz, onAnswer }: FlashQuizProps) {
  ...
  const checkAnswer = (e: FormEvent) => {
    ...
    const correctAnswer = quiz.front.trim().toLowerCase();
  ...
  }, [answered, readyForNext, quiz.id, isCorrect, onAnswer]);
  ...
  return (
    ...
    >
      <div className="absolute top-3 right-3 px-2 py-1 rounded-full bg-[var(--neutral-200)] text-xs flex items-center shadow-sm">
        <span className="mr-1">{getBoxEmoji(quiz.box_number)}</span>
        <span>훈련소 {quiz.box_number}</span>
      </div>
              ...
              <div className={`p-3 rounded-lg text-center font-medium ${
                ...
                {isCorrect ? (
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      정답은 <span className="font-bold">{quiz.front}</span> 입니다.
                    </div>
                  </div>
                )}
              </div>
